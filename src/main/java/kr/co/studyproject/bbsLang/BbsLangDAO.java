...
public class BbsLangDAO {
	...
	public int create(BbsLangDTO dto) {
		...
		try {
			...
			sql.append(
					" INSERT INTO bbsLang(wno, wtitle,wcontent,wpasswd, userid, lcode, wview, windent, wnum, ip, wdate, mdate) ");
	...
	public LinkedList<BbsLangDTO> search(String word, int start, int end) {
	...
	private LinkedList<BbsLangDTO> make_list(ResultSet rs, int start, int end) { // 본글과 답글의 순서 위아래 재배치
		try {
			...
			LinkedList<BbsLangDTO> temp2 = new LinkedList<>(); // 본글위치에 댓글의위치를 넣어주기 위해 ArrayList가 아니라 LinkedList사용 -
																// ArrayList보다 LinkedList가 삽입, 삭제에 유리
			...
			do {// ArrayList에 저장된 갯수만큼 LinkedList에 저장 하기위한 반복
				for (int i = 0; i < temp.size(); i++) {// ArrayList 사이즈만큼 반복
					if (temp.get(i).getWindent() == wind) { // ArrayList에 저장된 글이 본글인지 댓글인지 판단
						if (wind == 0)
							temp2.add(temp.get(i));// ArrayList에 저장된 글이 본글일경우 - wind가 0일때 저장하므로
